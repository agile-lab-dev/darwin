...
object HttpOptions {
  ...
  def allowUnsafeSSL: HttpOption = c =>
    c match {
  ...
  def sslSocketFactory(sslSocketFactory: SSLSocketFactory): HttpOption = c =>
    c match {
      case httpsConn: HttpsURLConnection =>
...
case class HttpRequest(
  ...
  def proxyAuth(user: String, password: String) =
    header("Proxy-Authorization", HttpConstants.basicAuthValue(user, password))
  ...
  private def toResponse[T](
    ...
    (if (
       responseCode == 401 && !this.headers
         .exists(p => p._1 == AuthHeaderName && p._2.startsWith(DigestAuth.DigestPrefix))
     ) {
      ...
      (if (
         conn.getInstanceFollowRedirects && (responseCode == 301 || responseCode == 302 || responseCode == 307 || responseCode == 308)
       ) {
         headers
           .get("Location")
...
object HttpConstants {
  ...
  val setFixedLengthStreamingMode: (HttpURLConnection, Long) => Unit = { case (connection, contentLength) =>
