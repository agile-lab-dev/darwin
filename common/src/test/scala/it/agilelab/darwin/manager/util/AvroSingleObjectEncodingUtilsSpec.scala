...
abstract class AvroSingleObjectEncodingUtilsSpec(val endianness: ByteOrder) extends AnyFlatSpec with Matchers {
  ...
  "isAvroSingleObjectEncoded(Array)" should "return true if the array contains only the header" in {
    AvroSingleObjectEncodingUtils.isAvroSingleObjectEncoded(Array(0xc3.toByte, 0x01.toByte)) should be(true)
  }
  ...
  "isAvroSingleObjectEncoded(Buffer)" should "return true if the array contains the header plus random bytes" in {
    ...
    AvroSingleObjectEncodingUtils.isAvroSingleObjectEncoded(Array(0xc3.toByte, 0x01.toByte) ++ buffer) should be(true)
  }
  ...
  "generateAvroSingleObjectEncoded(Array[Byte])" should "generate a single object encoded array" in {
    ...
    AvroSingleObjectEncodingUtils.generateAvroSingleObjectEncoded(
      ...
      testId,
      endianness
    ) should contain theSameElementsAs (Array(0xc3.toByte, 0x01.toByte) ++ testId.longToByteArray(endianness) ++ buffer)
  }
  ...
  "generateAvroSingleObjectEncoded(OutputStream)" should "generate a single object encoded array" in {
    ...
    AvroSingleObjectEncodingUtils
      ...
      .toByteArray should contain theSameElementsAs (Array(0xc3.toByte, 0x01.toByte) ++ testId.longToByteArray(
      endianness
    ) ++ random)
  }
  ...
  "AvroSingleObjectEncodingUtils" should "encode using streams" in {
    ...
    AvroSingleObjectEncodingUtils
      .generateAvroSingleObjectEncoded(stream, AvroSingleObjectEncodingUtils.getId(schema), endianness) { os =>
  ...
  "AvroSingleObjectEncodingUtils" should "using byte buffers" in {
    ...
    AvroSingleObjectEncodingUtils.generateAvroSingleObjectEncoded(
      stream,
      AvroSingleObjectEncodingUtils.getId(schema),
      endianness
    ) { os =>
      writer.write(record, encoder)
      writer.write(record, encoder)
